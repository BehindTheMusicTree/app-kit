import { BackendError, ConnectivityError } from "../transport/app-errors/app-error";
import { ErrorCode } from "../transport/app-errors/app-error-codes";
import { createAppErrorFromErrorCode } from "../transport/app-errors/app-error-factory";
...
export async function exchangeCodeWithBackend(
  ...
  const setBackendAuthError = () =>
    setConnectivityError(createAppErrorFromErrorCode(ErrorCode.BACKEND_AUTH_ERROR) as ConnectivityError);
  ...
  try {
    ...
    console.error("[auth] exchangeCodeWithBackend error", e);
    setBackendAuthError();
    return null;
  }
}
