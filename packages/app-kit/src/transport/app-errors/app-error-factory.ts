...
/**
 * `backendBaseUrl` is passed in by the caller (rather than read from a hardcoded app-specific
 * env var / site-urls helper) so this transport layer stays app-agnostic. Pass the same base URL
 * your `useFetchWrapper` was configured with; omit it and backend-vs-service error classification
 * degrades to always treating responses as "service" (non-backend) errors.
 */
export async function createAppErrorFromResult(result: Response, backendBaseUrl?: string): Promise<AppError> {
  const isBackendError = !!backendBaseUrl && result.url.includes(backendBaseUrl);
...
export function createNetworkOrBackendError(error: unknown, url: string, backendBaseUrl?: string): AppError {
  const isBackendError = !!backendBaseUrl && url.includes(backendBaseUrl);
